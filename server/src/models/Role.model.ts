...
export interface IRole extends Document {
  ...
  permissions: string[];
  /**
   * Tracks which default permissions have already been auto-granted by
   * `role.service.ts`'s reseed step, separate from `permissions` itself —
   * so re-running the seed only grants permissions newly ADDED to a role's
   * defaults, and never re-grants one an admin deliberately revoked.
   */
...
const roleSchema = new Schema<IRole>(
  {
    ...
    seededPermissions: [{ type: String, enum: PERMISSIONS }],
